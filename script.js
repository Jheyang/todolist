let categories = [
    ...
    if (categoryTasks.length === 0) {
      tasksContainer.innerHTML = `<p class="no-tasks">No tasks added for this category</p>`;
        ...
        categoryTasks.forEach((task) => {
            ...
            // Include task details with date and time
            div.innerHTML = `
              <div class="delete">
                <svg xmlns="https://www.w3.org/TR/SVG" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            ...
            label.innerHTML = `
              ...
              <p>${task.task}</p>
              <p class="due-date">Due: ${task.date} at ${task.time}</p>
            `;
  ...
  const addTask = (e) => {
    ...
    const taskDate = document.getElementById("task-date").value; // New input
    const taskTime = document.getElementById("task-time").value; // New input
  
    if (task === "") {
      alert("Please enter a task");
    } else if (taskDate === "" || taskTime === "") {
      alert("Please enter both date and time for the task");
    } else {
      const newTask = {
        id: tasks.length + 1,
        task,
        category,
        date: taskDate, // Add date
        time: taskTime, // Add time
        completed: false,
      };
      taskInput.value = "";
